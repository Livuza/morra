...
class Player extends React.Component {
  ...
  async getFingersAndGuess() { // Fun([], [UInt,UInt])
    const [ fingers, guess ] = await new Promise(resolveHandP => {
      this.setState({view: 'GetFingers', playable: true, resolveHandP});
    });
    this.setState({view: 'WaitingForResults', fingers});
    return [ parseInt(fingers), parseInt(guess) ];
  }
  seeOutcome(i) { this.setState({view: 'Done', outcome: intToOutcome[i]}); }
  notifyTimeout() { this.setState({view: 'Timeout'}); }
